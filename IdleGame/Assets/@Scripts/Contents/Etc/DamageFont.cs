...
public class DamageFont : MonoBehaviour
{
	...
	private void DoAnimation()
	{
		...
		seq.Append(transform.DOScale(1.3f, 0.3f).SetEase(Ease.InOutBounce)).		// 일감을 추가하는데
			Join(transform.DOMove(transform.position + Vector3.up, 0.3f).SetEase(Ease.Linear))	// 여기까지 묶어서 
